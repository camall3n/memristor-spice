...
time1 = (*(SPfrontEnd->IFseconds))();
	...
	if (! job->Df2wanted) {
        error = CKTnames(ckt,&numNames,&nameList);
        if(error) return(error);
        (*(SPfrontEnd->IFnewUid))((GENERIC *)ckt,&freqUid,(IFuid)NULL, "frequency", UID_OTHER,(GENERIC **)NULL);
        (*(SPfrontEnd->OUTpBeginPlot))((GENERIC *)ckt,(GENERIC*)ckt->CKTcurJob,"DISTORTION - 2nd harmonic",
